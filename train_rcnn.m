...
% imageFilenames = strcat(repmat({'resized/'}, [54,1]),data{1});
...
gpuDevice(1);
rcnn_p = trainRCNNObjectDetector(table_p, layers2, options, 'NegativeOverlapRange', [0 0.3]);
% save ('rcnn_p.mat', rcnn_p)

%%
% Test the R-CNN detector on a test image.
img = imread('cropped/test_eye.jpg'); 
% img = imread('resized/eye_05.jpg');
img_diff = imresize(img, [250 NaN]);
% img_d = gpuArray(img_diff);
% img_d = im2uint8(img_d);

[bbox, score, label] = detect(rcnn_p, img, 'MiniBatchSize',1);
%%
